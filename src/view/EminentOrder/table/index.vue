...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).EminentOrder){
        this.query = {
          user_name: JSON.parse(parameter).EminentOrder.user_name ? JSON.parse(parameter).EminentOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).EminentOrder.order_number ? JSON.parse(parameter).EminentOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).EminentOrder.category ? JSON.parse(parameter).EminentOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).EminentOrder.key ? JSON.parse(parameter).EminentOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).EminentOrder.timeArr ? JSON.parse(parameter).EminentOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).EminentOrder.start_time ? JSON.parse(parameter).EminentOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).EminentOrder.end_time ? JSON.parse(parameter).EminentOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).EminentOrder.orderby ? JSON.parse(parameter).EminentOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).EminentOrder.orderbyasc ? JSON.parse(parameter).EminentOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).EminentOrder.page)
      }else{
        this.getData()
      }
    },
