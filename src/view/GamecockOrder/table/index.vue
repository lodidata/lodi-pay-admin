<template>
  <div>
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        <div class="item">
          <span>{{i18n_lang.sportOrder0}}</span>
          <Input v-model="query.user_name" :placeholder="i18n_lang.sportOrder1" style="width: 120px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.sportOrder2}}</span>
          <Input v-model="query.order_number" :placeholder="i18n_lang.sportOrder3" style="width: 200px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.sportOrder4}}</span>
          <Select v-model="query.key" style="width:100px">
            <Option v-for="(game, i) in gameList" :key="i" :value="game.key">{{ game.name }}</Option>
          </Select>
        </div>
        <div class="item">
          <span>{{i18n_lang.sportOrder5}}</span>
          <!-- <DatePicker
            ...
            :placeholder="i18n_lang.sportOrder6"
            style="width: 200px"
          ></DatePicker> -->
      ...
      <div class="query_btns">
        <Button type="primary" icon="ios-search" style="margin-right: 5px;" @click="getData()">{{i18n_lang.sportOrder7}}</Button>
        <Button type="primary" icon="md-refresh" @click="reset">{{i18n_lang.sportOrder8}}</Button>
      </div>
    </Card>
    ...
    <div style="padding:10px 0 0; font-size: 16px;text-align: end;">{{ `${i18n_lang.sportOrder27}${page.total}${i18n_lang.sportOrder28}` }}</div>
    <Page
      class="page_box"
      show-sizer
      split-panels
    ...
    <!-- 订单详情 -->
    <Modal v-model="visible" :title="i18n_lang.sportOrder9" @on-visible-change="handleCloseModal">
      ...
      <div slot="footer">
        <Button type="primary" @click="visible = false">{{i18n_lang.sportOrder10}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    pageTotal() {
      ...
      total.desc = `${this.i18n_lang.sportOrder11}${hash.length}${this.i18n_lang.sportOrder12}`
      return total
    }
  },
  async created() {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).GamecockOrder){
        this.query = {
          user_name: JSON.parse(parameter).GamecockOrder.user_name ? JSON.parse(parameter).GamecockOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).GamecockOrder.order_number ? JSON.parse(parameter).GamecockOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).GamecockOrder.category ? JSON.parse(parameter).GamecockOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).GamecockOrder.key ? JSON.parse(parameter).GamecockOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).GamecockOrder.timeArr ? JSON.parse(parameter).GamecockOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).GamecockOrder.start_time ? JSON.parse(parameter).GamecockOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).GamecockOrder.end_time ? JSON.parse(parameter).GamecockOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).GamecockOrder.orderby ? JSON.parse(parameter).GamecockOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).GamecockOrder.orderbyasc ? JSON.parse(parameter).GamecockOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).GamecockOrder.page)
      }else{
        this.getData()
      }
    },
    async getData(i = 1) {
      ...
      if (res.attributes) {
        ...
        this.statistics = {
          desc: `${this.i18n_lang.sportOrder13}${res.attributes.total_user_count}${this.i18n_lang.sportOrder12}`,
