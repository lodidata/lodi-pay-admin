<template>
  <div>
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        <div class="item">
          <span>{{i18n_lang.buYuOrder1}}</span>
          <Input v-model="query.user_name" :placeholder="i18n_lang.buYuOrder2" style="width: 120px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.buYuOrder3}}</span>
          <Input v-model="query.order_number" :placeholder="i18n_lang.buYuOrder4" style="width: 200px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.buYuOrder5}}</span>
          <Select v-model="query.key" style="width:100px">
            <Option v-for="(game, i) in gameList" :key="i" :value="game.key">{{ game.name }}</Option>
          </Select>
        </div>
        <div class="item">
          <span>{{i18n_lang.buYuOrder6}}</span>
          <!-- <DatePicker
            ...
            :placeholder="i18n_lang.buYuOrder7"
            style="width: 200px"
          ></DatePicker> -->
      ...
      <div class="query_btns">
        <Button type="primary" icon="ios-search" style="margin-right: 5px;" @click="getData()">{{i18n_lang.buYuOrder8}}</Button>
        <Button type="primary" icon="md-refresh" @click="reset">{{i18n_lang.buYuOrder9}}</Button>
      </div>
    </Card>
    ...
    <div style="padding:10px 0 0; font-size: 16px;text-align: end;">{{ `${i18n_lang.buYuOrder29}${page.total}${i18n_lang.buYuOrder30}` }}</div>
    <Page
      class="page_box"
      show-sizer
      split-panels
    ...
    <!-- 订单详情 -->
    <Modal v-model="visible" :title="i18n_lang.buYuOrder10" @on-visible-change="handleCloseModal">
      ...
      <div slot="footer">
        <Button type="primary" @click="visible = false">{{i18n_lang.buYuOrder11}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    pageTotal() {
      ...
      total.TableID = `${this.i18n_lang.buYuOrder12}${hash.length}${this.i18n_lang.buYuOrder13}`
      return total
    }
  },
  async created() {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).BuYuOrder){
        this.query = {
          user_name: JSON.parse(parameter).BuYuOrder.user_name ? JSON.parse(parameter).BuYuOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).BuYuOrder.order_number ? JSON.parse(parameter).BuYuOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).BuYuOrder.category ? JSON.parse(parameter).BuYuOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).BuYuOrder.key ? JSON.parse(parameter).BuYuOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).BuYuOrder.timeArr ? JSON.parse(parameter).BuYuOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).BuYuOrder.start_time ? JSON.parse(parameter).BuYuOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).BuYuOrder.end_time ? JSON.parse(parameter).BuYuOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).BuYuOrder.orderby ? JSON.parse(parameter).BuYuOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).BuYuOrder.orderbyasc ? JSON.parse(parameter).BuYuOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).BuYuOrder.page)
      }else{
        this.getData()
      }
    },
    async getData(i = 1) {
      ...
      if (res.attributes) {
        ...
        this.statistics = {
          TableID: `${this.i18n_lang.buYuOrder14}${res.attributes.total_user_count}${this.i18n_lang.buYuOrder13}`,
