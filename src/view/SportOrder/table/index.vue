...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).SportOrder){
        this.query = {
          user_name: JSON.parse(parameter).SportOrder.user_name ? JSON.parse(parameter).SportOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).SportOrder.order_number ? JSON.parse(parameter).SportOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).SportOrder.category ? JSON.parse(parameter).SportOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).SportOrder.key ? JSON.parse(parameter).SportOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).SportOrder.timeArr ? JSON.parse(parameter).SportOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).SportOrder.start_time ? JSON.parse(parameter).SportOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).SportOrder.end_time ? JSON.parse(parameter).SportOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).SportOrder.orderby ? JSON.parse(parameter).SportOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).SportOrder.orderbyasc ? JSON.parse(parameter).SportOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).SportOrder.page)
      }else{
        this.getData()
      }
    },
