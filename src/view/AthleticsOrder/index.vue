...
export default {
  ...
  methods: {
    ...
    async getMenu() {
      ...
      const data = res.data.find(o => o.type === 'ESPORTS')
      this.menu = data.childs && data.childs.length ? data.childs : []
      let parameter = sessionStorage.getItem('parameter') || '{}'
      this.curTab = JSON.parse(parameter).AthleticsOrder ? JSON.parse(parameter).AthleticsOrder.type_name : this.menu[0].type
    }
  }
}
</script>
