...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).AthleticsOrder){
        this.query = {
          user_name: JSON.parse(parameter).AthleticsOrder.user_name ? JSON.parse(parameter).AthleticsOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).AthleticsOrder.order_number ? JSON.parse(parameter).AthleticsOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).AthleticsOrder.category ? JSON.parse(parameter).AthleticsOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).AthleticsOrder.key ? JSON.parse(parameter).AthleticsOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).AthleticsOrder.timeArr ? JSON.parse(parameter).AthleticsOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).AthleticsOrder.start_time ? JSON.parse(parameter).AthleticsOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).AthleticsOrder.end_time ? JSON.parse(parameter).AthleticsOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).AthleticsOrder.orderby ? JSON.parse(parameter).AthleticsOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).AthleticsOrder.orderbyasc ? JSON.parse(parameter).AthleticsOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).AthleticsOrder.page)
      }else{
        this.getData()
      }
    },
