...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).ElectronicOrder){
        this.query = {
          user_name: JSON.parse(parameter).ElectronicOrder.user_name ? JSON.parse(parameter).ElectronicOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).ElectronicOrder.order_number ? JSON.parse(parameter).ElectronicOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).ElectronicOrder.category ? JSON.parse(parameter).ElectronicOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).ElectronicOrder.key ? JSON.parse(parameter).ElectronicOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).ElectronicOrder.timeArr ? JSON.parse(parameter).ElectronicOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).ElectronicOrder.start_time ? JSON.parse(parameter).ElectronicOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).ElectronicOrder.end_time ? JSON.parse(parameter).ElectronicOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).ElectronicOrder.orderby ? JSON.parse(parameter).ElectronicOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).ElectronicOrder.orderbyasc ? JSON.parse(parameter).ElectronicOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).ElectronicOrder.page)
      }else{
        this.getData()
      }
    },
