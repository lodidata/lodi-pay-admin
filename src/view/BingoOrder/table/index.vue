...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).bingoOrder){
        this.query = {
          user_name: JSON.parse(parameter).bingoOrder.user_name ? JSON.parse(parameter).bingoOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).bingoOrder.order_number ? JSON.parse(parameter).bingoOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).bingoOrder.category ? JSON.parse(parameter).bingoOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).bingoOrder.key ? JSON.parse(parameter).bingoOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).bingoOrder.timeArr ? JSON.parse(parameter).bingoOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).bingoOrder.start_time ? JSON.parse(parameter).bingoOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).bingoOrder.end_time ? JSON.parse(parameter).bingoOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).bingoOrder.orderby ? JSON.parse(parameter).bingoOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).bingoOrder.orderbyasc ? JSON.parse(parameter).bingoOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).bingoOrder.page)
      }else{
        this.getData()
      }
    },
