<template>
  <div>
    ...
    <Table ref="table" border :columns="loginList" :data="dataSource" @on-sort-change="sortTable"></Table>
    ...
    ></Page>
    <Modal v-model="visible.details" :title="`${i18n_lang.ipRankingList7}`" :width="1200" :footer-hide="true">
...
<script>
  ...
  import { loginList, detailsList} from './columnsLogin'
  export default {
    data() {
    ...
    methods: {
      ...
      async detail(index = 1){
        ...
        const res = await this.$axios.request({
          url: '/report/iprank/login/detail',
          method: 'GET',
          params: {
      ...
      async exportDataAjax(){
        const res = await this.$axios.request({
          url: '/report/iprank/login/export',
          method: 'GET',
          responseType: 'blob',
          params: {
            ...this.searchInfo
