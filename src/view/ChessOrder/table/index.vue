...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).ChessOrder){
        this.query = {
          user_name: JSON.parse(parameter).ChessOrder.user_name ? JSON.parse(parameter).ChessOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).ChessOrder.order_number ? JSON.parse(parameter).ChessOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).ChessOrder.category ? JSON.parse(parameter).ChessOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).ChessOrder.key ? JSON.parse(parameter).ChessOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).ChessOrder.timeArr ? JSON.parse(parameter).ChessOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).ChessOrder.start_time ? JSON.parse(parameter).ChessOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).ChessOrder.end_time ? JSON.parse(parameter).ChessOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).ChessOrder.orderby ? JSON.parse(parameter).ChessOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).ChessOrder.orderbyasc ? JSON.parse(parameter).ChessOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).ChessOrder.page)
      }else{
        this.getData()
      }
    },
