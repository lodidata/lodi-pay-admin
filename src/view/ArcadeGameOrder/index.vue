...
export default {
  ...
  methods: {
    ...
    async getMenu() {
      ...
      const data = res.data.find(o => o.type === 'ARCADE')
      this.menu = data.childs && data.childs.length ? data.childs : []
      let parameter = sessionStorage.getItem('parameter') || '{}'
      this.curTab = JSON.parse(parameter).ArcadeGameOrder ? JSON.parse(parameter).ArcadeGameOrder.type_name : this.menu[0].type
    }
  }
}
</script>
