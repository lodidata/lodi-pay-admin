<template>
  <div>
    ...
    <Card dis-hover class="query_box">
      <div class="query_con">
        <div class="item">
          <span>{{i18n_lang.chessOrder0}}</span>
          <Input v-model="query.user_name" :placeholder="i18n_lang.chessOrder1" style="width: 120px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.chessOrder2}}</span>
          <Input v-model="query.order_number" :placeholder="i18n_lang.chessOrder3" style="width: 200px" />
        </div>
        <div class="item">
          <span>{{i18n_lang.chessOrder4}}</span>
          <Select v-model="query.key" style="width:100px">
            <Option v-for="(game, i) in gameList" :key="i" :value="game.key">{{ game.name }}</Option>
          </Select>
        </div>
        <div class="item">
          <span>{{i18n_lang.chessOrder5}}</span>
          <!-- <DatePicker
            ...
            :placeholder="i18n_lang.chessOrder6"
            style="width: 200px"
          ></DatePicker> -->
      ...
      <div class="query_btns">
        <Button type="primary" icon="ios-search" style="margin-right: 5px;" @click="getData()">{{i18n_lang.chessOrder7}}</Button>
        <Button type="primary" icon="md-refresh" @click="reset">{{i18n_lang.chessOrder8}}</Button>
      </div>
    </Card>
    ...
    <div style="padding:10px 0 0; font-size: 16px;text-align: end;">{{ `${i18n_lang.chessOrder27}${page.total}${i18n_lang.chessOrder28}` }}</div>
    <Page
      class="page_box"
      show-sizer
      split-panels
    ...
    <!-- 订单详情 -->
    <Modal v-model="visible" :title="i18n_lang.chessOrder9" @on-visible-change="handleCloseModal">
      ...
      <div slot="footer">
        <Button type="primary" @click="visible = false">{{i18n_lang.chessOrder10}}</Button>
      </div>
    </Modal>
  </div>
</template>
...
export default {
  ...
  props: ['type'],
  data() {
    const i18n_lang = this.$t('chessOrder');
    return {
      i18n_lang: i18n_lang,
      page: {
        pageSize: 20,
  ...
  computed: {
    ...
    pageTotal() {
      ...
      total.TableID = `${this.i18n_lang.chessOrder11}${hash.length}${this.i18n_lang.chessOrder12}`
      return total
    }
  },
  async created() {
  ...
  methods: {
    ...
    init() {
      ...
      if(JSON.parse(parameter).ArcadeGameOrder){
        this.query = {
          user_name: JSON.parse(parameter).ArcadeGameOrder.user_name ? JSON.parse(parameter).ArcadeGameOrder.user_name : '', // 会员账号
          order_number: JSON.parse(parameter).ArcadeGameOrder.order_number ? JSON.parse(parameter).ArcadeGameOrder.order_number : '', // 注单号
          category: JSON.parse(parameter).ArcadeGameOrder.category ? JSON.parse(parameter).ArcadeGameOrder.category : '', // 筛选列表
          key: JSON.parse(parameter).ArcadeGameOrder.key ? JSON.parse(parameter).ArcadeGameOrder.key : '', // 筛选子id
          timeArr: JSON.parse(parameter).ArcadeGameOrder.timeArr ? JSON.parse(parameter).ArcadeGameOrder.timeArr : [formatDateToString(todayStart), formatDateToString(todayEnd)],
          start_time: JSON.parse(parameter).ArcadeGameOrder.start_time ? JSON.parse(parameter).ArcadeGameOrder.start_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 00:00:00`, // 开始时间
          end_time: JSON.parse(parameter).ArcadeGameOrder.end_time ? JSON.parse(parameter).ArcadeGameOrder.end_time : `${new Date().getFullYear()}-${new Date().getMonth() + 1}-${new Date().getDate()} 23:59:59`, // 结束时间
          orderby: JSON.parse(parameter).ArcadeGameOrder.orderby ? JSON.parse(parameter).ArcadeGameOrder.orderby : '',
          orderbyasc: JSON.parse(parameter).ArcadeGameOrder.orderbyasc ? JSON.parse(parameter).ArcadeGameOrder.orderbyasc : ''
        }
        this.getData(JSON.parse(parameter).ArcadeGameOrder.page)
      }else{
        this.getData()
      }
    },
    async getData(i = 1) {
      ...
      if (res.attributes) {
        ...
        this.statistics = {
          TableID: `${this.i18n_lang.chessOrder13}${res.attributes.total_user_count}${this.i18n_lang.chessOrder12}`,
