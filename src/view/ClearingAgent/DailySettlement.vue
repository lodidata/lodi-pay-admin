...
<script>
  ...
  import { collectData, agentDetails } from './dailyColumns'
  const searchInfo = {
    model: 1,
    user_name: '',
    agent_name: '',
  ...
  export default {
    ...
    created() {
      ...
      if(JSON.parse(parameter).DailySettlement){
        this.searchInfo = {
          model: 1,
          user_name: JSON.parse(parameter).DailySettlement.user_name, // 用户名
          agent_name: JSON.parse(parameter).DailySettlement.agent_name,
          date_start: JSON.parse(parameter).DailySettlement.date_start,
          date_end: JSON.parse(parameter).DailySettlement.date_end,
        }
        this.page.currentPage = JSON.parse(parameter).DailySettlement.page
        this.page.pageSize = JSON.parse(parameter).DailySettlement.page_size
