<template>
  <div id="SubMenu" class="card">
    <Button
      ...
      @click="backGameManage"
    >
{{ type === 'c' ? i18n_lang.gameManage7 : i18n_lang.gameManage8 }}
</Button>
    ...
    <Modal v-model="visible.modal" :title="i18n_lang.gameManage9" :width="600" :styles="{ top: '20px' }">
      ...
      <p class="title">{{i18n_lang.gameManage10}}</p>
      <UploadImg ref="uploadImg" v-model="gameIcon">
        <p>{{i18n_lang.gameManage11}}</p>
      </UploadImg>

      <template v-if="gameId===15||gameId===47">
        <p class="title">{{i18n_lang.gameManage12}}</p>
        <UploadImg ref="uploadImg" v-model="qp_img2">
          <p>{{i18n_lang.gameManage13}}</p>
        </UploadImg>
      </template>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      gameId: Number(this.$route.query.id),
      columns: [],
      dataSource: [],
      game: '', // 游戏所属
      pid: '', // 一级菜单 id
  ...
  methods: {
    ...
    async swapItems(current, target) {
      ...
      this.loading.page = true
      const req = this.dataSource.map((o, i) => ({ id: o.id, across_sort: i + 1 }))
      const res = await this.$axios.request({
    ...
    handleEditIcon(row) {
      ...
      this.editId = row.id
      this.gameIcon = row.qp_img
      this.qp_img2 = row.qp_img2
    },
    // 确认按钮
    async handleConfirm() {
    ...
    async sureEditIcon() {
      ...
      const res = await this.$axios.request({
        ...
        data: {
          id: this.editId,
          type: this.type,
          qp_img: this.gameIcon,
          qp_img2: this.qp_img2
        }
      })
      this.loading.modalButton = false
